...
const getFollowFeedsUser = `-- name: GetFollowFeedsUser :many
SELECT id, created_at, updated_at, user_id, feed_id FROM feeds_follows
WHERE user_id = $1
`

func (q *Queries) GetFollowFeedsUser(ctx context.Context, userID uuid.UUID) ([]FeedsFollow, error) {
	rows, err := q.db.QueryContext(ctx, getFollowFeedsUser, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
